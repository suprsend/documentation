...
We recommend **using profiles** — your token stays out of shell history and you can switch between workspaces easily:
...
<Accordion title="Other authentication methods">
  **Environment variable** — useful for scripts and ephemeral environments:

  ```bash
  export SUPRSEND_SERVICE_TOKEN="your_service_token_here"
  ```

  **Command flag** — useful when starting the MCP server directly:

  ```bash
  suprsend start-mcp-server --service-token "your_service_token_here"
  ```

  Note: passing tokens as flags may expose them in shell history. Profiles or environment variables are preferred for anything persistent.
</Accordion>
...
- [Global Flags](/reference/cli-global-flags) — flags available across all commands
- [Workflow commands](/reference/cli-workflow) — list, pull, push, enable, and disable workflows
- [CLI Changelog](/reference/cli-changelog) — see what's new