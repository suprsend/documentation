...
Set it as an environment variable so all CLI commands can use it automatically:
...
<Accordion title="Other authentication methods">
  **Command flag** — useful for one-off commands or when starting the MCP server directly:

  ```bash
  suprsend start-mcp-server --service-token "your_service_token_here"
  ```

  Note: passing tokens as flags may expose them in shell history. Environment variables are preferred for anything persistent.
</Accordion>
