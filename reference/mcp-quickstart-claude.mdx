...
<Tip>
  **Easiest path on Claude Code:** install the [SuprSend Agent Plugin](/reference/agent-plugin) instead. One command (`/plugin install suprsend@suprsend-marketplace`) gets you the MCP server *and* all SuprSend Skills in a single step. The manual setup below is for users who want raw MCP only.
</Tip>
