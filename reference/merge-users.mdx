...
When you merge two users, there's a primary user passed in your API path parameter and merge a secondary / old user passed as `from_user_id` in your payload. The primary `distinct_id` is retained after the merge and the `from_user_id` is deleted. The deletion process is permanent and `from_user_id` can't be recovered once deleted.
