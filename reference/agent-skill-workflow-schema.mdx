...
Your agent loads the skill and produces a file like this in `suprsend/workflow/payment-failed.json`:
...
A few real workflows people generate with this skill:

- **Cart abandonment** — wait 1 hour, then email and in-app notification.
- **Password reset** — immediate email with retry SMS after 5 minutes.
- **Onboarding drip** — email today, in-app tomorrow, SMS in 3 days unless the user activates.
- **Payment reminder** — branch on invoice status, escalate channels over a week.
- **Re-engagement** — triggered when a user enters a "dormant" list.
