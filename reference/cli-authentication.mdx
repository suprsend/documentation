...
description: "Set up SuprSend CLI authentication using service tokens, including how to generate tokens in the dashboard and the priority order the CLI uses to resolve them."
keywords: ["suprsend cli authentication", "suprsend service token", "suprsend cli setup", "suprsend environment variables", "suprsend cli configuration"]
---
...
<Info>
  **Self-hosted or BYOC deployment?** These two methods work for SuprSend Cloud (SaaS), where the CLI connects to default SuprSend endpoints. If you are running a self-hosted or BYOC deployment with custom API endpoints, you'll need to set up CLI profiles to tell the CLI where your servers are. See [Setting Up CLI Access](/docs/self-hosted/cli-profiles).
  ...
  Profiles are not related to workspaces. The `-w` flag (e.g., `-w production`) selects a workspace. Profiles configure which _server_ the CLI connects to.
</Info>
