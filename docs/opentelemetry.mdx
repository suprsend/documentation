---
title: "OpenTelemetry"
description: "Stream real-time notification metrics from SuprSend to any OpenTelemetry-compatible observability platform via OTLP for API, delivery, and vendor tracking."
---
...
<Steps>
  ...
  <Step title="Configure connector settings">
    ...
    | Field | Description |
    |-------|-------------|
    | **Connector Name** | A label to identify this connector (for example `Grafana Cloud Production`) |
    | **Tool Used** | The name of the observability tool you are connecting to (for example `Grafana Cloud`) |
    | **OTLP Endpoint** | Your platform's OTLP HTTP endpoint URL (for example `https://otlp-gateway-prod-us-east-0.grafana.net/otlp/v1/metrics`) |
