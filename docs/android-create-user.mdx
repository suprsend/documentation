---
title: "Manage Users"
description: "Use the SuprSend Android SDK to create and identify users, set push tokens, attach email or SMS channels, and manage user profile properties from your app."
---
...
<Steps>
  <Step title="Create/Identify a new user" stepNumber="1">
    ...
    | --- | --- | --- |
    | **distinct\_id** | int, bigint, string, UUID | _mandatory_ Unique identifier for a user across devices or between multiple logins. |
  </Step>
...
<AccordionGroup>
  <Accordion title="Set" defaultOpen={false}>
    ...
    | Parameters | Type | Description |
    | --- | --- | --- |
    | **key** | string | _Mandatory_ This is property key that will be attached to user. Should not start with `$` or `ss_` |
    | **value** | any | _Optional_ This will be value that will be attached to key property. |
    | **JSONObject** | object | _Optional_ This is used in case of setting multiple user properties. |
  ...
  <Accordion title="Set Once" defaultOpen={false}>
    Works just like `ssApi.getUser().set`, except it will not overwrite existing property values. This is useful for properties like _First login date._
