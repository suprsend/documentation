...
2. Select **Use guided mode**, then choose **Write your own query**.
3. Enter the following NRQL query:
   ```sql
   SELECT sum(suprsend.messages.delivery_errors) FROM Metric SINCE 5 minutes ago
   ```
4. Click **Next** to proceed to **Set thresholds**.
5. Under **Threshold type**, select **Static**.
...
3. Enter the following NRQL query:
   ...
   ```
4. Click **Next** to proceed to **Set thresholds**.
5. Under **Threshold type**, select **Anomaly**.
