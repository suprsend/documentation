...
Once translation files are updated, pass a `locale` parameter (e.g., `es`, `fr`, `de`) when loading the [embeddable preference center](/docs/user-preferences#embed-in-your-product) or pass locale as query parameter in the [get user preference API](/reference/get-user-full-preference).
