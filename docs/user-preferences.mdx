...
We support **[23 languages](#supported-languages)** with automatic locale detection using internationalization (i18n). The page uses smart fallback—regional locales like `es-MX` fall back to base language `es`, then to English (`en`).

**How translations work:**
- **Static content** (buttons, labels) translates automatically based on user's locale
- **Dynamic content** (category names, descriptions) uses translation files you upload

<Accordion title="Supported languages (23)" defaultOpen={false} id="supported-languages">
...
Once uploaded, pass a `locale` parameter (e.g., `es`, `fr`, `de`) when loading the [embeddable preference center](/docs/user-preferences#embed-in-your-product) or as a query parameter in the [get user preference API](/reference/get-user-full-preference). The [hosted preference page](/docs/user-preferences#hosted-preference-page) detects locale automatically.
