...
description: "Learn how user preferences work in SuprSend and how to capture per-user channel and category opt-ins to honor consent across every notification you send."
---

<Note>
  **Before you start:** Make sure you've set up preference categories first. See [Manage Categories and Preferences](/docs/managing-notification-categories) for step-by-step instructions.
</Note>
...
SuprSend provides [ready-made UI components](/docs/user-preferences#embed-in-your-product) and [APIs](/reference/overview) to build preference screens where users can opt in or opt out of different preference categories and channels. This works for both B2B and B2C applications.
...
1. **`channel_preferences`** — Global channel opt-outs (for example, opt out of all email)
...
<AccordionGroup>
  <Accordion title="On - Users receive this category's notifications by default">
    ...
    - **Selected Channels only**: Only specific channels you select are enabled by default (for example, Email, Android Push, iOS Push, In-App Inbox, MS Teams, Slack)
  </Accordion>
...
## Digest schedule and conditions

Beyond on/off and channel preferences, a sub-category can carry two additional, optional settings that you configure when [managing categories](/docs/managing-notification-categories):

- **Digest schedule** — lets users pick how often they receive batched notifications for a sub-category (for example, *Instant*, *Daily at 9 AM*, or *Weekly on Monday*) right from their preference page. You define the available cadence options; the option a user selects drives the workflow [Digest node](/docs/digest) at send time.
- **Condition properties** — custom, configurable values you attach to a sub-category and then reference inside workflow [branch conditions](/docs/branch) or **trigger condition**. Use them to control *when* a category's notifications are sent — for example, a minimum severity, an alert threshold, or which user roles should be notified.

Like other preferences, the resolved value follows the **User → Tenant → Category default** precedence: a user's selection wins, falling back to the tenant default, then the category default.

For setup, see [Configuring digest schedule](/docs/managing-notification-categories#configuring-digest-schedule) and [Configuring condition properties](/docs/managing-notification-categories#configuring-condition-properties).

### Digest schedule

Each subcategory in the [preference API](/reference/get-user-full-preference) response includes `digest_schedule_options` (all available options for the preference UI) and `digest_schedule` (the resolved active schedule for the user).

<CodeGroup>
```json GET preference response
{
...
```
```json Update user preference
// PATCH /v1/user/{distinct_id}/preference/category/{category_slug}/
// Only pass id and fields with edit_policy: "editable"
{
  "digest_schedule": {
    "id": "daily-summary",
    "time": "10:00"
...
Set `digest_schedule` to `null` in the update request to fall back to the tenant or category default. Only fields with `edit_policy: "editable"` can be overridden by the user.

### Condition properties

Each subcategory in the [preference API](/reference/get-user-full-preference) response includes a `properties` array with the configured condition properties and the user's current values.
...
```json Update user preference
// PATCH /v1/user/{distinct_id}/preference/category/{category_slug}/
// Only pass key and value for properties with edit_policy: "editable"
{
  "properties": [
    {
      "key": "threshold",
...
<Frame>
  <img src="/images/hosted-preference-page.png" />
</Frame>
...
In a multi-tenant setup, tenants or admins can control which categories their users see. Setting `visible_to_subscriber: false` in [tenant preferences](/reference/update-tenant-preference-single-category) hides the category from tenant users' preference pages. Hidden categories won't send notifications to those users, even if they previously opted in.
...
Tags can be added to sections and sub-categories directly from **Developers → Preference Categories** in the SuprSend Console. When a tag is assigned at the section level, it automatically applies to all categories under that section-so filtering by a section tag also filters its child categories.
<Frame>
  <img src="/images/preference-tag-changelog.png" alt="Tags Within Preference Categories" />
</Frame>

</Step>

<Step title="Filter Categories with Tags">
You can filter categories using the tags query parameter in the API. This can be a simple tag match (for example `tags=tag1`) or a more advanced filter using logical operators.
...
- Once uploaded, pass a `locale` parameter (for example, `es`, `fr`, `de`) when:
...
SuprSend evaluates **category-level** preference opt-outs at **trigger time** — if a user has opted out of a category, the workflow is not executed for that user. **Channel-level** preferences are evaluated at the delivery node when the notification is about to be sent. In both cases, the system checks user-level preferences first, then tenant-level overrides, and finally category defaults.

For detailed information on the evaluation process, see [Preference Evaluation](/docs/preference-evaluation).

---

## Other ways to unsubscribe from notifications
...
<AccordionGroup>
  <Accordion title="Email: Unsubscribe URL header" icon="envelope-dot" iconType="solid">
    ...
    <Frame>
      <img src="/images/docs/96222900be2910eeaae1edfb7bd8e532ef2b090d828616ac87409085cb1bd89a-image.png" />
    </Frame>
  </Accordion>
  ...
  <Accordion title="Mobile Push: Preference Page in App settings" icon="mobile-screen" iconType="solid">
    ...
    <Frame>
      <img src="/images/docs/0a8782302c5613a1a952b362912789007197bf449b8c5888e9857470840a1865-image.png" />
    </Frame>
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Can users control when they receive digest notifications?">
    Yes. The simplest way is to enable a **digest schedule** on the sub-category. Define cadence options (for example, Instant, Daily, Weekly), and users pick their preferred one from the preference page. In your workflow's [Digest node](/docs/digest), set the schedule type to `category_digest_schedule` and SuprSend sends each recipient's digest on the cadence they chose. See [Configuring digest schedule](/docs/managing-notification-categories#configuring-digest-schedule).

    Alternatively, if you want full control over the schedule object passed at runtime, store the preferred schedule as a custom property in the user profile and reference it in a [dynamic digest schedule](/docs/digest#dynamic-schedule-send-digest-based-on-user-preference):
    ```json
    user.set({
      "digestSchedule": {
        "frequency": "daily",
  ...
  <Accordion title="Why do users still receive promotional notifications even after unsubscribing from all categories?">
    ...
    **Solution:** Create sub-categories under the Promotional category (for example, "Marketing", "Newsletter", "Product Updates") and use those sub-category slugs in your workflows.
  ...
  <Accordion title="How do I let users control both notification on/off and the time they want to be reminded?">
    ...
    **1. Set up preference categories:**
    - Create a preference category (for example, "meeting-reminders") that users can opt in/out of using the [preference APIs](/reference/update-user-category-preference) or [preference center UI](/docs/user-preferences#capturing-user-preferences).
    ...
    **3. Use dynamic schedule in digest node:**
    - In your workflow's digest node, configure it to use a [dynamic schedule](/docs/digest#dynamic-schedule-send-digest-based-on-user-preference) that references the user property (for example, `."$recipient".meetingReminderTime`).
...
- [Preference Categories](/docs/notification-category) - Setting up categories & defaults
