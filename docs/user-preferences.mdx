...
### **Controlling Category Visibility Using Preference Tags**

Filtering notification categories based on custom-defined **Tags** in Preference Categories. It gives firm control over category visibility to your end-users.​ The capabilities includes:

  - **Tag-Based Filtering**: Assign single or multiple tags to preference sub-categories to tailor notification delivery.
    ![Tags Within Preference Categories](/images/preference-tag-changelog.png)

    <Accordion title="Filtering basic Tags in Preferences">
      For any given tag `abcd`:

      We use plain direct filter as: `tags=abcd` 
      
      This will returns the row categories associated with the tag "abcd".    
    </Accordion>

    <Accordion title="Filtering Complex Logic Tags in Preferences">
      To deal with complicated logics, we use the operators: `or`, `and`, `not`, `exists`.
      
      Creating a JSON-based expression for the same.

      | Tag Filter | Description |
      |-----|-----|
      |`tags={"exists": true}`|only return rows that have some tag set|
      |`tags={"exists": false}`|only return rows that no tag set|
      |`tags={"or": ["abcd", "cdef", ...]}`|return rows that have either of the mentioned tags |
      |`tags={"and": ["abcd", "cdef",...]}`|return rows that have both the mentioned tags|
      |`tags={"not": "admin"}`|return rows that don't have admin tag|
      
      <Note>
      - You can nest operators as per need.
      - For logical operators `or`, `and`, `not`, the operands can be either a string or a nested condition.
      </Note>

      </Accordion>

  - **Logical Operators**: Utilize logical operators (`and`, `or`, `not`, `exists`) to create [complex filtering](/docs/user-preferences#filtering-complex-logic-tags-in-preferences) logic, based on your ecosystem.

  - **Nested Conditions**: In case of a complex-ecosystem of notification, and considering the tenant's UI, combine the logical operators to form nested conditions for advanced filtering scenarios.

> **Example for logical-nested conditions:**
>
> ```json
> tags={"or":[{"and":["check1","abcd"]},{"and":["check1",{"not":"check2"}]}]}
> ```
>
> This sample logic returns categories that either:
> - Have both `check1` & `abcd` tags.  
> - Or have `check1` but **do not** have the `check2` tag.
>
> Such a use-case will **show or hide categories** based on user **roles, subscription level, or team assignment** — all dynamically controlled using tags.
