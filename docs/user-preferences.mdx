...
You'll first need to upload translation files for the locale you want to translate to. See [How to manage Category translations](/docs/managing-notification-categories#managing-category-translations) for details.

Once translation files are updated, pass a `locale` parameter (e.g., `es`, `fr`, `de`, `es-AR`) when calling the `getPreferences` method in the web SDK. Since embeddable SDKs (React, Angular) are built on the web SDK base, you pass the locale through the web SDK's `getPreferences` method - no separate locale parameter is needed for embeddable SDKs.

**Example:**
```typescript
// In JavaScript/Web SDK
const preferencesResp = await suprSendClient.user.preferences.getPreferences({
  locale: 'es' // or 'fr', 'de', 'es-AR', etc.
});
```

Alternatively, you can pass locale as a query parameter in the [get user preference API](/reference/get-user-full-preference).
