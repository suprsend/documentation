...
<Warning>
  **Preferences only work with sub-categories:** User preferences apply to sub-categories you create (e.g., "Marketing", "Newsletter"), not top-level categories (System, Transactional, Promotional). Use sub-category slugs in workflows for preferences to work.
</Warning>
...
Users can set their preferences through multiple methods:
...
**Category translations:** Pass a `locale` parameter (e.g., `es`, `fr`, `de`) when loading the preference center to display translated category and section names. The system automatically falls back using this logic: `locale-region` → `locale` → `en`. This enables users to see preference categories in their preferred language, improving user experience and engagement. See [Category translations](/docs/notification-category#category-translations) for details.
...
2. **Filter categories with tags**: Use tags to show categories based on user roles, departments, or teams. Filter categories in the preference center using the `tags` query parameter.
...
<AccordionGroup>
  ...
  <Accordion title="Can I use user preferences in workflow branching to control which notifications are sent?">
    ...
    **Workaround:** If you need to use preference-based logic in workflows (e.g., to route notifications based on user preferences or combine multiple notification scenarios in a single workflow), you can:
    
    1. Store the same preference data as custom properties in the user profile
    2. Use those custom properties in [branch conditions](/docs/branch) to route notifications
  ...
  <Accordion title="How do I let users control both notification on/off and the time they want to be reminded (e.g., medicine reminders)?">
    You can combine preference categories with dynamic digest schedules to achieve this:

    **1. Set up preference categories:**
    - Create a preference category (e.g., "medicine-reminders") that users can opt in/out of using the [preference APIs](/reference/update-user-category-preference) or [preference center UI](/docs/user-preferences#capturing-user-preferences).
    ...
    **3. Use dynamic schedule in digest node:**
    - In your workflow's digest node, configure it to use a [dynamic schedule](/docs/digest#dynamic-schedule-send-digest-based-on-user-preference) that references the user property (e.g., `."$recipient".medicineReminderTime`).
