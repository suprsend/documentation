...
| `{{$batched_events.[0].name}}` | Print the value of the nth element of the batch events array. 0 refers to first element, 1 refers to second element, etc.|
| `{{$recipient.name}}` | Print the value of the recipient property `name`. |
| `{{$actor.name}}` | Print the value of the actor property `name`. |
| `{{$tenant.name}}` | Print the value of the tenant reserved properties like name. |
| `{{$tenant.properties.address}}` | Print the value of the tenant custom property like address, phone number, etc. |
...
Can be used to show some content in the template based on a condition. For email, you can use [display condition](/docs/email-template#display-conditions) to show or hide a particular content block
...
<Tip>
  **AI prompt - debug rendering:** *"My SuprSend template isn't rendering. Template: [paste]. Variables JSON: [paste]. Expected: [describe]. Actual: [blank/raw syntax/error]. Check for: path mismatch, double vs triple braces for URLs, #each on non-array, missing closing tags, missing $ on $recipient/$tenant."*
</Tip>
