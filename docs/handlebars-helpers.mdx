---
title: "Handlebars"
description: "Handlebars syntax and list of supported handlebars helpers that can be used in a template to format data."
---
...
<Accordion title="How does SuprSend avoid HTML injection when printing variables?">
  SuprSend uses the default syntax for variables `{{ var }}` to print the value of the variable. This is **HTML-escaped**, so characters like `<`, `>`, `"`, `'`, `&` are converted into random strings (like &amp;, ??, =??, etc.) when they are rendered as text.
</Accordion>
...
Use these prompts with ChatGPT, Claude, or your IDE's AI assistant to write or debug Handlebars expressions.

<AccordionGroup>
  <Accordion title="Write a Handlebars expression for a specific task">
    ```text
    Write a Handlebars expression for SuprSend templates that:
    [describe what you want — e.g.
    - "shows a comma-separated list of product names from an array"
    - "displays a fallback message when the variable is empty"
    - "formats a date as 'Mon DD, YYYY'"
    - "shows different content for premium vs free users"]

    My variable structure:
    [paste your JSON payload from the Variables panel]

    SuprSend supports standard Handlebars helpers plus custom helpers
    like {{formatDate}}, {{math}}, {{capitalize}}, {{default}},
    {{formatCurrency}}.
    ```
  </Accordion>

  <Accordion title="Debug a Handlebars template that isn't rendering">
    ```text
    My SuprSend template isn't rendering correctly.

    Template: [paste the broken section]
    Variables JSON: [paste from Variables panel]
    Expected: [what you expected]
    Actual: [what you're seeing — blank, raw syntax, error]

    Check for: path mismatch, {{}} vs {{{}}} for URLs, #each on
    non-array, missing closing tags, missing $ prefix on $recipient
    or $brand.
    ```
  </Accordion>
</AccordionGroup>