...
### Variable syntax and injection prevention

SuprSend uses [Handlebars](https://handlebarsjs.com/) as the templating language. How you reference variables affects whether their values are escaped, which helps prevent injection in emails and other channels.

| Syntax | Behavior | When to use |
| :----- | :------- | :---------- |
| `{{ var }}` | **HTML-escaped** (default). Characters like `<`, `>`, `"`, `'`, `&` are escaped so they render as text, not as HTML or script. | **Recommended.** Use for any user-provided or dynamic content (names, messages, links, etc.) to avoid XSS and injection. |
| `{{{ var }}}` | **Raw output.** The value is inserted as-is with no escaping. | Use only when you intentionally need unescaped HTML (e.g. trusted, sanitized content). Using raw output with untrusted data poses a security risk. |

**Best practice:** Prefer `{{ var }}` for variables in templates. Use `{{{ var }}}` only when you explicitly need unescaped HTML and control the source of the data.
