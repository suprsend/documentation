...
### Create SuprSend account
  ...
  ### Create MS Teams App
    To activate MS Teams Integration, start from [Microsoft Teams](/docs/microsoft-teams) and pick a use case: [webhook](/docs/microsoft-teams-incoming-webhook), [your bot](/docs/microsoft-teams-customer-workspaces), or [the customer's bot](/docs/microsoft-teams-customer-bot).
...
<br></br>
    **Sample payload for API based trigger**
    ...
    You can get workspace key, secret or API Key for trigger from [Settings tab -> API Keys ](https://app.suprsend.com/en/sandbox/developers/api-keys). Here, we are defining MS teams channel inline using `conversation_id` and `tenant_id`. Copy those IDs from Teams as described in [Send as your bot](/docs/microsoft-teams-customer-workspaces#for-a-first-test-copy-ids-by-hand). `service_url` is optional; omit it to use Microsoft's global endpoint. Example regional service URL-
