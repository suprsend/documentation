...
description: "Quick start to configure a Microsoft Teams bot, integrate it with SuprSend, and trigger your first chat notification on MS Teams workspaces."
---
...
### Create SuprSend account
  ...
  ### Create MS Teams App
    To activate MS Teams Integration, start from [Microsoft Teams](/docs/microsoft-teams) and pick a use case: [webhook](/docs/microsoft-teams-incoming-webhook), [your Teams app](/docs/microsoft-teams-customer-workspaces), or [your customer's Teams app](/docs/microsoft-teams-customer-bot).
...
<br></br>
    **Sample payload for API based trigger**
    ...
    You can get workspace key, secret or API Key for trigger from [Settings tab -> API Keys ](https://app.suprsend.com/en/sandbox/developers/api-keys). Here, we are defining MS teams channel inline using `conversation_id` and `tenant_id`. Copy those IDs from Teams as described in [Send using your Teams app](/docs/microsoft-teams-customer-workspaces#4-save-teams-channels-and-users-in-suprsend). `service_url` is optional; omit it to use Microsoft's global endpoint. Example regional service URL-
