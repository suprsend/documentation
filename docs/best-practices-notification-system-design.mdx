...
* **Use Test Mode for safe testing in staging workspace**: Enable [Test Mode](/docs/developer-tools/test-mode) in your workspace to safely test notification flows without delivering to real users. In Test Mode, notifications to real users are blocked and delivery is allowed only to designated internal testers. You can also set up a catch-all channel to redirect all notifications intended for non-test users.
