...
The channel ID of notification is picked automatically from the workflow's preference category used to trigger the notification in SuprSend. If you want to customise notifications of specific notification channel then pick its id from workflow you are triggering define it's properties like above snippet else they will fallback to default notification channel.
