...
- **[Expiry](#expiry):** Add a 15-day (or longer) expiry to most notifications to keep inboxes clean. Use shorter durations for time-sensitive content like webinars or flash sales. Enable the expiry timer to drive urgency.

<Note>
  Avoid adding expiry to long-lived notifications that users might reference later, like product updates or blog posts.
</Note>

- **[Pinning](#pin-notification):** Reserve for notifications requiring immediate user action — compliance tasks, critical system updates, urgent plan renewals. Always combine pinning with an expiry so the notification doesn't stay pinned indefinitely.
