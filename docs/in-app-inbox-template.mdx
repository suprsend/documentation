  ...
  <Step title="Using variables in the templates">
    ...
    <Steps>
      <Step title="Enter a nested variable">
        To enter a nested variable, enter in the format `{{var1.var2.var3}}`. for example: to refer to city in the example above, you need to enter `{{event.location.city}}`
      </Step>

      <Step title="Refer to an array element">
        To refer to an array element, enter in format `{{var1.[*index*].var2}}. for example: to refer to `product\_name`of the first element of the array`array, enter `` {{array.[0].product_name}}` ``
      </Step>
