...
> ⚠️ Use caution: deleting PVCs will erase all stored data if reclaim policy is not set to `Retain`.
...
> **Important**: This section shows only the **ClickHouse-specific** configuration. You must also configure other required secrets and values for SuprSend to work properly. See the complete configuration guide: [SuprSend Installation Guide](/docs/self-hosted/suprsend-installation-guide)
