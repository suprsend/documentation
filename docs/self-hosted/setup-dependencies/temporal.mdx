...
Now modify `values/values.postgres.yaml` 

<details>
<summary>Sample values/values.postgres.yaml</summary>
```yaml title="values/values.postgres.yaml"
server:
  config:
    persistence:
      default:
...
and install Temporal using Helm:
...
<Steps>
<Step title="Verify if the namepsaces are created properly">
...
<Step title="Check if the attributes are created properly">
