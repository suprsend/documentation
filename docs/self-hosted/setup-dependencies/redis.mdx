...
description: "Provision and operate the two Redis-compatible stores (cache and worker state) for SuprSend self-hosted deployments."
---
...
## Step 2: Minimum Requirements & Versioning
...
## Step 3: Instance Sizing (Starting Points)
...
## Step 4: Connection & App Configuration

Set the following environment variables in SuprSend services:

<Note>
Since we use DB numbers for isolation, only pass redis url without `/` and database number for the Cache Store.
</Note>
...
## Step 5: Recommended Configurations

### Step 5.1: Cache Store (Standalone Redis, non-cluster)
...
### Step 5.2: Worker State Store (Redis)
...
### Step 5.3: Worker State Store (KV-Rocks)
...
## Step 10: Observability and Backup
...
## Step 11: Managed Services (Recommended)
...
## Step 12: Cost Optimization Tips
...
## Step 13: SuprSend Helm Configuration

Once your Redis instances are set up and running, configure SuprSend to use them.

<Note>
This section shows only the **Redis-specific** configuration. You must also configure other required secrets and values for SuprSend to work properly. See the complete configuration guide: [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide)
</Note>

### Kubernetes Secret Configuration

First, add the Redis-specific secrets to your SuprSend secrets file (see [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide) for the full secrets template):
...
<CardGroup cols="2">
  <Card title="OpenSearch Setup Guide" icon="magnifying-glass" iconType="solid" href="/docs/byoc/setup-dependencies/opensearch">
    Set up OpenSearch for search and analytics.
  </Card>
