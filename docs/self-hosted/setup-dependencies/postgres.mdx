...
> **Note**: Verify `pg_cron` availability. If unavailable, consider self-hosted deployment.
...
> **Important**: This section shows only the **PostgreSQL-specific** configuration. You must also configure other required secrets and values for SuprSend to work properly. See the complete configuration guide: [SuprSend Installation Guide](/docs/self-hosted/suprsend-installation-guide)
...
> **Note**: SuprSend will automatically connect to the required databases (`suprsend`, `pronto`, `svix`, `defaultdb`) using the same host and credentials. Ensure all databases are created on the same PostgreSQL instance. The above configuration goes under `global.config` section.
