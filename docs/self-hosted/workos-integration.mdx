...
- Your dashboard URL (for example `https://app.yourdomain.com`) and API URL (for example `https://api.yourdomain.com`)
...
| **AuthKit base URL** | Dashboard → Authentication → AuthKit → your hosted domain (for example `https://your-app.authkit.app`) |
...
suprsendapi:
  config:
    ...
    # WorkOS AuthKit hosted base URL (for example https://your-app.authkit.app)
...
<AccordionGroup>
  ...
  <Accordion title="Why do I see LINK_INVALID after returning from WorkOS?">
    - Check that the redirect URI in the WorkOS dashboard **exactly** matches your callback URL (for example `https://app.yourdomain.com/en/callback`).
