---
title: "Tenants"
description: "Create, update, fetch, and list SuprSend tenants from your backend using the Java SDK, with code samples for white-labeling and per-tenant personalization."
---
...
| Field                                 | Description                                                                                                                                                               |
| :------------------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `tenant_id` (mandatory)                 | max 64 characters and can contain lowercase letters (a-z), digits (0-9), dot (.), hyphen (-), and underscore (_). tenant_id is case-insensitive. Suprsend will first converts tenant_id to lowercase before storing it or doing any sort of comparison on it.                                                                    |
...
| `social_links`                          | URLs of social media accounts of the tenant. While updating a social link, if you pass its value as null, then SuprSend will ignore it. If you really want to remove the value you must the `value= " "` (instead of null). for example If you want to remove social_link url for `facebook`, you must pass `facebook= " "`. Consider this code block for the same:`{ "social_links": {"facebook": " " } }`|
