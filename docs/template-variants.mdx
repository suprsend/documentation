...
Open any template, then click the **variants** icon in the top-right toolbar (next to **Exit** and **Commit**).

<Frame caption="Variants button in the top-right toolbar of the template editor">
  <img src="/images/docs/templates-variants-toolbar-button.png" width="360" />
</Frame>

The **Create Variant** modal opens. Fill in the fields below and click **Create**. Once created, the variant appears in the **All Variants** panel on the left side of the editor, where you can select, reorder, duplicate, or delete it.
...
**Conditions** *(required)*
Conditions define **when this variant should be picked** at send time. If a variant has no conditions, SuprSend can't tell it apart from the default and will never send it — so every non-default variant needs at least one.

Combine conditions using **AND** and **OR**:

- **AND** — stack rows inside the same group. Every row must match for the group to pass (e.g. `plan == "pro"` **AND** `country == "US"`).
- **OR** — click **+ OR** to add a new group. The variant is picked if **any** group passes (e.g. `plan == "pro"` **OR** `cohort intersects ["beta"]`).

<Note>
  **Locale** and **Tenant** count as conditions too. If you've set either, the variant already has at least one matching rule — you only need to add explicit **Conditions** rows when also targeting payload data, recipient properties, or actor properties.
</Note>
...
<Frame caption="Editing a French locale variant for the In-App Inbox channel — variants panel on the left, editor in the middle, live preview on the right">
  <img src="/images/docs/templates-variant-inapp-inbox-multilingual.png" />
</Frame>
