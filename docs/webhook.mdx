...
Webhook node supports JSONNET rendering language. You can add workflow trigger payload variables as `data.<key>` and internal suprsend data as `data["$brand"].<key>`. Here's a list of variables supported in workflow engine:
