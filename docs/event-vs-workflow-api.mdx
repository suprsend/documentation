...
description: "Compare the event trigger and direct workflow API methods in SuprSend to choose the right approach for triggering notification workflows from your backend."
---
...
| **User pre-creation** | Mandatory - event is silently discarded if user doesn't exist | Not mandatory - supports [defining recipient inline](/docs/trigger-workflow#identifying-recipients-inline) (acts as UPSERT). Also supports [anonymous/transient recipients](/docs/python-trigger-workflow-from-api#sending-notification-to-anonymous-user) via `is_transient: true` |
...
| **Object triggers** | Supported via [override recipient](/docs/override-recipient-list) - point to an event property containing the `object_type` and `object_id` | Supported natively - pass `object_type` and `object_id` directly in the recipients array. Recommended approach for sending to non-user entities like [objects](/docs/objects) |
| **One event → multiple workflows** | Yes - a single event can fan out to many workflows| No - each API call targets one workflow|
| **Payload schema validation** | [Supported](/docs/validate-workflow-payload) - validates event properties with the defined schema and throws error in the API response if validation fails | [Supported](/docs/validate-workflow-payload) - validates workflow data with the defined schema and throws error in the API response if validation fails |
...
| Notifications to objects (departments, teams, topics) | Either - Workflow API for direct targeting, Event via [override recipients](/docs/override-recipient-list) when object ID is in event data |
...
Both event properties and workflow API payloads support runtime validation via [JSON schemas](/reference/upsert-schema). Schema Validation catches payload mismatches at the API level before the workflow executes.
