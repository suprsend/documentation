...
| channels (Optional)   | string[]                                            | User channels on which the broadcast messages to be sent. If not provided, it will trigger notifications on all available channels in user profile. Available channels: Android Push / IOS Push / Inbox / Email / WhatsApp / SMS Example:["sms", "inbox"] |
