...
description: "Upgrade the SuprSend Web SDK between major versions with this migration guide."
---
...
- `changeTenant(tenantId)` to switch the active tenant of an identified user without resetting the session. This is meant for user tokens that scope multiple tenants (`scope.tenant_id` as an array). Already running feed instances and previously fetched preferences keep the tenant they were initialized with — re-initialize the feed and call `getPreferences` again after switching. Refer [docs](/docs/integrate-javascript-sdk#change-active-tenant).
