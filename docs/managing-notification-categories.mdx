...
<Note>
  ...
  <Tab title="Via Management API">
    Use Management API to programmatically add, update, or delete translations for specific locales. Translations go live immediately after upload.

    **Get Translation**

    Fetch existing translations for a specific locale:

    ```bash
    curl -X GET "https://management-api.suprsend.com/v1/{workspace}/preference_category/translation/content/es/" \
    ...
    **Add or Update Translation**
    ...
    Add new translations or update existing ones for a locale. The API will merge your changes with existing translations:

    ```bash
    curl -X POST "https://management-api.suprsend.com/v1/{workspace}/preference_category/translation/content/es/" \
      --header "Authorization: ServiceToken {token}" \
      --header "Content-Type: application/json" \
      --data '{
        "sections": {
          ...
          "marketing": {
            "name": "Marketing",
            "description": "Notifications marketing et promotions"
          }
        },
        "categories": {
          "invoice-ready": {
    ...
    **Python example:**
    ...
    get_url = f"https://management-api.suprsend.com/v1/{workspace}/preference_category/translation/content/{locale}/"
    headers = {
        "Authorization": f"ServiceToken {service_token}",
        "Content-Type": "application/json"
    }
    
    response = requests.get(get_url, headers=headers)
    existing_translations = response.json()
    ...
    post_url = f"https://management-api.suprsend.com/v1/{workspace}/preference_category/translation/content/{locale}/"
    response = requests.post(post_url, headers=headers, json=translations)
    ...
    **Important notes:**
    - Replace `{workspace}` with your workspace slug (e.g., `staging`, `production`)
    - Replace `{token}` with your Service Token (found in [Workspace Settings](https://app.suprsend.com/en/staging/settings/workspace))
    - The `locale` parameter supports standard locale codes (e.g., `es`, `fr`, `de`, `es-AR`)
    - You can update partial translations—only the sections/categories you include will be updated
    - Set `description` to `null` if you want to remove a description
...
<AccordionGroup>
  ...
  <Accordion title="How many categories should I create?">
    ...
    - **Recommended**: 5-15 sub-categories total across all root-categories
    ...
    **Best practice**: Create categories based on user actions or notification purposes (e.g., "Order Updates", "Account Security", "Marketing Newsletters") rather than creating a category for every single workflow.
  </Accordion>
  ...
  <Accordion title="Why aren't user preferences working for my notifications?">
    ...
    **Common issue**: If you're sending notifications using only a root-category (e.g., `notification_category: "promotional"`), preferences won't apply and all users will receive notifications regardless of their preference settings.
    
    **Solution**: 
    1. Create a sub-category (e.g., "Marketing", "Newsletter", "Product Updates") under the appropriate root-category
    2. Use the sub-category slug in your workflows (e.g., `notification_category: "marketing"`)
    3. Publish your category changes
    
    Once you use a sub-category, user preferences will be respected and unsubscribed users won't receive notifications in that category.
  </Accordion>
  ...
  <Accordion title="My user has opted out of all categories, but they are still receiving notifications">
    ...
    **Root-categories** (System, Transactional, Promotional) don't respect user preferences. If you're sending notifications with `notification_category: "promotional"` or `notification_category: "transactional"`, users will continue to receive notifications even after opting out.
    
    **To fix this**:
    1. Check your workflow configuration and ensure you're using sub-category slugs (e.g., `"marketing"`, `"order-updates"`) instead of root-category names
    2. Verify that the sub-categories are published (categories must be published to appear in workflows)
    3. Test by triggering a workflow and checking if the notification respects user preferences
    
    **Note**: System category notifications are designed to always be sent (users can't unsubscribe), so this behavior is expected for critical notifications like OTPs and password resets.
  </Accordion>
  ...
  <Accordion title="How do I show users only the categories relevant to their role or department?">
    ...
    **Example**: If you tag "Sales" categories with `sales` and "Engineering" categories with `engineering`, you can filter preferences by passing `tags=sales` or `tags=engineering`.
