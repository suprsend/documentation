  ...
  <Tab title="Dashboard">
    <Steps>
      ...
      <Step title="Check it against a real person">
        ...
        with their actual data. Variables that don't resolve show in **red** — a red variable is a
        blank space in someone's inbox.
...
Open the list you sent to. Two tabs, two views:

| Tab | Shows |
|---|---|
| **Broadcast Executions** | The send as a whole — how many were triggered, skipped and failed |
| **Messages** | One row per recipient, with the template and channel each person got |

| Status | Meaning |
|---|---|
| **Triggered** | Reached on at least one channel. This is the number you want |
| **Skipped** | Not sent — the user opted out, or has no address on that channel |
| **Failed** | The send broke, usually a template that couldn't render |

<Frame>
  {/* SCREENSHOT NEEDED: broadcast-logs.png — the Broadcast Executions tab on a list, showing the triggered / skipped / failed counts for a send */}
  <img src="/images/broadcast-logs.png" alt="The Broadcast Executions tab showing the triggered, skipped and failed counts" />
</Frame>

<Check>
  Triggered matches the size of your list, and the message is in the inbox of the address you
  verified.
</Check>
...
<AccordionGroup>
  ...
  <Accordion title="Failed with a template rendering error">
    The template used a variable the broadcast didn't supply. Names in **Variable data** have to match
    the template exactly, and `{{$recipient.<property>}}` only resolves for users who have that
    property on their profile.

    Open the template, set a real user as the Recipient in the Variables panel, and look for
