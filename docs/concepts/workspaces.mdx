...
<Frame>
  <img src="/images/docs/d44c8b5993f4dace694f63d10493f76c81814c25a23ddf5642d26e6d2253460a-Screenshot_2024-09-08_at_9.26.14_PM.png" />
</Frame>
...
<Steps>
  <Step title="Always test in staging before production">
    Build and validate workflows, templates, vendor configuration, and preference setup in staging. Use [Test Mode](/docs/developer/test-mode) to redirect deliveries to internal testers while you iterate. Only promote changes to production once thoroughly tested.
  </Step>

  <Step title="Avoid making direct changes in production">
    Direct edits in production can disrupt live notifications. Always use clone, CLI, or Management API to promote vetted changes. Restrict production write access through the **Member** role for everyone except a small set of trusted admins.
  </Step>

  <Step title="Keep credentials environment-specific">
    Use separate workspace keys, secrets, and API keys per environment in your application's environment variables (`.env.staging`, `.env.production`, or a secrets manager). Never reuse production credentials in lower environments.
  </Step>

  <Step title="Configure vendors per workspace">
    Add live vendor credentials only in production. Use vendor sandbox/test credentials in staging where the vendor supports it (e.g., AWS SNS Sandbox, Mailgun Sandbox Domain, Twilio test credentials). This protects against accidental sends and unwanted billing during development.
  </Step>

  <Step title="Update webhook URLs and secrets per workspace">
    SuprSend webhook URLs (for delivery tracking from SMS, Email, WhatsApp vendors) and the Inbox HMAC secret are workspace-specific. Make sure your production deployments point to the production webhook URL and use the production Inbox secret. See the [Go-live checklist](/docs/go-live-checklist).
  </Step>

  <Step title="Codify assets and promote via CI/CD">
    For mature teams, use the [SuprSend CLI](/reference/cli-intro) to track workflow, template, schema, and category definitions in Git, and promote them via your CI pipeline. This gives you version history, code review, and repeatable rollouts.
  </Step>

  <Step title="Use tenants, not workspaces, for per-customer customization">
    If you only need per-customer branding, vendor routing, or default preferences, use [tenants](/docs/tenants) inside a single workspace. Reserve separate workspaces for environments and genuinely independent business lines that don't share users or workflows.
  </Step>

  <Step title="Monitor each workspace independently">
    Logs, analytics, and audit logs are workspace-scoped. Set up alerts and review the [Logs page](https://app.suprsend.com/en/staging/logs/api?last_n_minutes=1440) for each production workspace right after launch and during high-traffic periods.
  </Step>
</Steps>
