...
description: "Manage tenant-level notification preferences in SuprSend for B2B and multi-tenant SaaS products, letting admins control channels for users in their tenant."
---
...
As a platform administrator, you can define which preference categories and channels are available to different tenants. For example, if you're a SaaS platform with paid features and some clients haven't subscribed to those features, you can hide preference categories related to the paid features for those tenants.
...
![](/images/tenant-pref.png)
</Frame>

## Tenant admin: Setting defaults for their users

Tenant administrators can set [default preferences](/reference/update-tenant-preference-single-category) for their end users and hide categories that aren't applicable to their users. This gives tenant admins control over which notifications are sent to their users and which categories appear on their users' preference pages.
...
<CardGroup cols="2">
  <Card title="Via API" icon="code" iconType="solid">
    Use the <a href="/reference/update-tenant-preference-single-category">tenant preference API</a> to programmatically manage preferences
  </Card>
...
### Overriding digest schedule and condition defaults

If a sub-category has a [digest schedule](/docs/managing-notification-categories#configuring-digest-schedule) or [condition properties](/docs/managing-notification-categories#configuring-condition-properties), tenant admins can override their defaults for the tenant's users — for example, set a different default digest cadence or a different default condition value (such as a minimum severity) for everyone in the tenant.

These values resolve with the same **User → Tenant → Category default** precedence: a user's own selection wins, then the tenant default, then the category default. The available options themselves are still defined on the category — a tenant can change the default, not add or remove options.

#### Digest schedule

The [tenant preference API](/reference/get-tenant-preference-single-category) response includes `digest_schedule.options` (tenant-overridden defaults) and `digest_schedule.original_options` (category-level originals). Tenants can change which option is the default and override editable field values like `time`, but cannot add or remove options.

<CodeGroup>
```json GET tenant preference response
{
  "digest_schedule": {
    "options": [
      {
...
```
```json Update tenant preference
// PATCH /v1/tenant/{tenant_id}/preference/category/{category_slug}/
// Reference option by id, override default_value for editable fields
{
  "digest_schedule": {
    "options": [
      {
...
Set `digest_schedule` to `null` in the update request to reset to category defaults.

#### Condition properties

The [tenant preference API](/reference/get-tenant-preference-single-category) response includes the `properties` array with the tenant's overridden values. `is_overridden` indicates whether the tenant has set a custom value.
...
<Tabs>
  ...
  <Tab title="Tenant Admin Actions">
    - Set `visible_to_subscriber: false` in [default tenant preferences](/reference/update-tenant-preference-single-category) to hide categories from tenant users' preference pages.
