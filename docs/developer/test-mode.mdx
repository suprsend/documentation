---
title: "Test Mode"
description: "Enable test mode in SuprSend to preview and debug workflow runs, templates and channel delivery safely without sending notifications to real users."
---
...
<Steps>
  ...
  <Frame>
    <img src="/images/testmode-enabled.png" alt="Test Mode Enabled" />
  </Frame>
  ...
  <Step title="Set a catch-all channel [Optional]">
    Assign one **catch-all channel per type** (for example, email, SMS).  
    All notifications sent to non-test users on that channel will be redirected here (for example, `qa@company.com`, `dev@company.com`).
    
    <Frame>
      <img src="/images/testmode-catchall.png" alt="Test Mode Catch-All" />
    </Frame>
