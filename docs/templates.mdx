...
<Note>
**Variable syntax:** Double curly braces `{{ var }}` HTML-escape values by default, which helps [prevent injection](/docs/handlebars-helpers#variable-syntax-and-injection-prevention). Use triple curly braces `{{{ var }}}` when you need raw output—for example, URLs or variables that may contain special characters (e.g. `{{{url}}}`), since double braces would escape those characters. Only use triple braces with data you control; raw output from untrusted data can pose a security risk.
</Note>
