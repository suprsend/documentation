...
For per-user personalization, you can use user custom properties in your templates. These properties are automatically available from each user's profile and can be referenced using `$user` or `$recipient` variables.

**Example:**
```
Hi {{$user.name}}! We noticed you're in {{$user.city}}.
```

User properties are automatically pulled from each user's profile. Set them when [creating or updating user profiles](/docs/users). This is particularly useful for [broadcasts](/docs/broadcast) where you want personalized content for each user while using generic variable data for campaign-wide information.
