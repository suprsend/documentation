...
### Select the Authentication Scheme
  To send email through SES, you can opt for either of the two schemes: **IAM User**, or **IAM Role**. If you opt for Role-based Authentication, start by creating a role that uses [trust policy](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_roles_create_for-user.html).
    ...
    <Accordion title="Creating IAM Role (recommended)" defaultOpen={false}>
        ...
        <Steps>
          <Step title="Create Policy:">
              ...
              * In the `Policies` tab, click on `Create Policy`: Either use a 'Visual-editor' or a 'JSON editor' to allow actions ``["ses:SendEmail", "ses:SendRawEmail"]``.
              ...
              * After click on `Next`, and enter a Policy Name (e.g. suprsend_trust_role_policy), & other optional Description.

              * Finally, clicking on the `Create Policy` button, will create a new policy with the above permissions.
          </Step>
          <Step title="Create IAM Role:">        
              * Go to IAM console & slect the `Roles` tab.
              ...
              * Click `Next` and them select the Policy you created in previous step (e.g. suprsend_trust_role_policy) and select it.
              
              * Further, clicking `Next`, enter the role name (e.g. suprsend_trust_role).

              * This will create a new role with the above permissions.
          </Step>
          <Step title="Verify the Role permissions:">
              ...
              * Go to the Trust Relationships tab, and you'd find a similar JSON to this:
                <CodeGroup>
                    ```json IAM Role Policy
                    {
                      "Version": "2012-10-17",
...
After getting started with the Authentication theme, you can proceed with the following next steps to set up your Amazon SES account with SuprSend:
