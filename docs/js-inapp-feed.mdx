---
title: "InApp Feed"
description: "Integrate the SuprSend in-app feed in non-React JavaScript apps using headless SDK methods to fetch notifications, mark read, and render a custom UI."
---
...
`tenantId` defaults to the active tenant set in [identify](/docs/integrate-javascript-sdk#authenticate-user) or [changeTenant](/docs/integrate-javascript-sdk#change-active-tenant), else the `default` tenant. Passing it here overrides the active tenant for that feed instance. Already running feed instances keep the tenant they were initialized with when `changeTenant` is called — re-initialize the feed to reflect the new tenant.

If you are passing `tenant_id` in feed, make sure to pass scope key while creating [userToken](/docs/client-authentication#2-creating-signed-user-jwt-token) else error will be thrown due to scope mismatching.
