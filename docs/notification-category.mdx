...
Transactional notifications are the ones that are sent in response to some user transaction or action on your platform. By default, users are subscribed to transactional notifications and not allowed to unsubscribe as they have initiated them at the first place. However, some of the transactional notifications are not so critical and can be unsubscribed as per user's wish (for example: Post likes/ comments). You can set which transactional notifications are critical and should not be allowed to unsubscribe by setting the default preference as `cant unsubscribe`.
