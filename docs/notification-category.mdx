...
Transactional notifications are the ones that are sent in response to some user transaction or action on your platform. Some examples are - Delivery updates, Payment Confirmation, booking confirmation, Post shared / liked, Balance alerts, account update alert, and reminders.
...
* System category default preference is set to `Can't Unsubscribe`. We recommend selecting at least one mandatory channel in system category since users should receive notifications on at least one channel in this category.

* The default preference for the Transactional category is set to `Can’t Unsubscribe` because these notifications typically contain essential, time-sensitive information. That said, you’re free to adjust the default preference for this category based on your requirements.

* The default preference for the Promotional category is set to `ON`, meaning users will receive notifications in this category by default. For categories that are optional, we recommend setting the default to `OFF` so users only receive those notifications if they explicitly opt in.
...
| Default Preference | *Mandatory* | Default notification preference for this sub-category. **`On`**: Users receive notifications by default. You can configure which channels are enabled via **Opt-in Channels**. **`Off`**: Users do not receive notifications unless they opt in. **`Can't Unsubscribe`**: Users cannot completely opt out. Configure **Mandatory Channels** (always sent) and default preference for non-mandatory channels (users can opt out, but default to "On"). |
| Opt-in Channels    | *Optional*  | Available when Default Preference is set to `On`. Specifies which channels are included in the default "On" state: - `All`: All available channels are enabled by default - `Selected Channels only`: Only specific channels you select are enabled by default (e.g., Email, Android Push, iOS Push, In-App Inbox, MS Teams, Slack)                                                                                                                                                                                                                                                                                                                           |
| Mandatory Channels | *Optional*  | Available when Default Preference is set to `Can't Unsubscribe`. Choose which channels are always sent (mandatory). For all other channels, configure whether they default to ON or OFF.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
