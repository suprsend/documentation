...
<Note>
  Templates are updated via management API proxy. If you're on a self-hosted deployment, pass `host="self-host-management-api-path"` (your self-hosted management API URL) instead of the standard API reference path.
</Note>
