...
e.g. - if the [segment track](https://segment.com/docs/connections/spec/track/) API call looks like this
...
<Steps>
  ...
  <Step title="Add Destination Name">
    On the setup screen, add Destination name. Destination name can be **"SuprSend \{workspace}"**. e.g. - For staging workspace, the destination name can be "SuprSend Staging." Choose **"Fill in settings manually"** and click on **"Create destination"**.
