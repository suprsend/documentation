...
<Steps>
  ...
  <Step title="Using variables in the templates">
    ...
    2. To refer to an array element, enter in format `{{var1.[*index*].var2}}. For example: to refer to `product\_name`of the first element of the array`array`, enter `\{\{array.\[0].product\_name}}\`
    ...
    * Make sure you have entered the correct variable name in the template, as per the `handlebarsjs` guideline.
    ...
    <Warning>
      **What happens if there is variable mismatch at the time of sending?**
      ...
      Please note that the rest of the templates will be sent. For example: if there is an error in rendering iOS Push template, but email template is successfully rendered, iOS Push notification will not be triggered, but email notification will be triggered by SuprSend.
    </Warning>
  </Step>
</Steps>
