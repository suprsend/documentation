...
Bulk operations use **UPSERT** to create or update users. The SDK automatically chunks requests based on user properties and identities.
...
<Tip>
  **Rate limit:** 1000 requests per second. The SDK automatically chunks requests (e.g., 20,000 users ≈ 100-200 API requests).
</Tip>
