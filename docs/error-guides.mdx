...
description: "Look up SuprSend workflow and broadcast execution error codes, understand their root causes and follow recommended steps to resolve each issue."
---
...
| Error | Solution |
|-------|----------|
| **NotificationCategory `&lt;category_slug&gt;` not found** | The preference category passed in workflow payload is not present on SuprSend dashboard or you have mistakenly added category name rather than its slug. Click on Preference category name to copy the slug. Please note only published categories can be used in workflows. If you have your category available in draft version, publish the changes before triggering the workflow. Navigate to [Settings -> Preference Categories](https://app.suprsend.com/en/staging/settings/notification-categories) to see all available categories. Click on Category name to copy slug. |
...
| **NotificationCategory `&lt;category_slug&gt;` not applicable to tenant `&lt;tenant_id&gt;`** | This means that tenant's default preference for the `category_slug` is turned off for `tenant_id`. To check default preference setting of each tenant, navigate to [default preferences tab](https://develop.suprsend-app.pages.dev/en/staging/brands/default/brand_preference) on tenant details page. |

### Tenant/Brand Errors

| Error | Solution |
|-------|----------|
| **tenant doesn't exist with id: `&lt;tenant_id&gt;`** | The `tenant_id` doesn't exist in the given SuprSend Workspace. Check for available tenants on [SuprSend dashboard -> tenants page](https://app.suprsend.com/en/production/tenants/). You can create this tenant from [SuprSend dashboard or via your backend SDK / API](/docs/tenants#creating--updating-tenant-on-suprsend-platform). |
...
| **no vendor configured for channel: and category: `&lt;notification_category&gt;` and id-provider:** | This happens when vendor configuration for the given channel and preference category is missing. Navigate to vendor page on SuprSend dashboard to setup vendor. You can find integration guidelines for all vendors [here](/docs/vendors). |
...
| Error | Solution |
|-------|----------|
| **SubscriberList Broadcast API: trigger_at must be in ISO 8601 format....** | This error occurs when time-duration format in the `trigger_at` key is incorrect. The correct format ISO 8601 timestamp. for example - `2024-01-01T05:30:00Z` indicates that the notification will be triggered at 1 Jan, 2024 5:30 AM UTC. |
| **SubscriberList Broadcast API: no vendor configured for channel: `&lt;channel_name&gt;` and category: `&lt;notification_category&gt;` and id-provider: `&lt;vendor_name&gt;`** | This happens when vendor configuration for the given channel and preference category is missing. Navigate to vendor page on SuprSend dashboard to setup vendor. You can find integration guidelines for all vendors [here](/docs/vendors). |
