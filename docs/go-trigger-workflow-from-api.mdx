...
| `delivery`              | Delivery instructions for the workflow. You can enable smart delivery by setting `"smart":True` By default, delivery instruction will be `"delivery": { "smart": False, "success": "seen" }` Checkout the [docs](/docs/go-trigger-workflow-from-api#smart-delivery) for more details. | `delivery = { "smart": True/False, "success": "delivered/seen/interaction/", "time_to_live": "", "mandatory_channels": [] }` # list of mandatory channels e.g gation", | _Optional_  |
