...
1. Category-level preference settings (Sections)
   ...
   - [Category Channel](/docs/js-preferences#13-category-channels-sections---sub-categories---channels)
     ...
     <img
       src="/images/docs/ios-sections-preferences.png"
       alt="Description"
       style={{
         width: "100%",
         height: "350px",
...
<img
  src="/images/docs/ios-channel-preferences.png"
  alt="Description"
  style={{
    width: "100%",
    height: "350px",
  ...
  func fetchAll() {
    ...
    Task {
      let result = await SuprSend.shared.preferences.getPreferences(
        args: Preferences.Args(tenantId: ""))
      DispatchQueue.main.async {
...
struct CategoryView: View {
  ...
  var body: some View {
    Section(
      header: Toggle(category.name, isOn: $isOn)
        .onChange(
          of: isOn,
          perform: { value in
            print("Toggle value: \(value)")
            _ = SuprSend.shared.preferences.updateCategoryPreference(
              category: category.category,
              preference: value ? PreferenceOptions.optIn : PreferenceOptions.optOut)
          }
        )
