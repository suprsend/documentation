---
title: "Preferences"
description: "Build a notification preference center in your iOS Swift app with SuprSend SDK methods to fetch, display, and update user channel and category opt-ins."
---
...
<Frame>
  <img src="/images/docs/mobile-sections.png" />
</Frame>
...
<Frame>
  <img src="/images/docs/mobile-category.png" />
</Frame>
...
| --------- | ----------- |
| `tenantId` | Tenant identifier for loading per-tenant preferences. Defaults to the active tenant set in `identify` or [changeTenant](/docs/ios-integration#change-active-tenant) |
...
<Warning>
If you are passing `tenantId` in preferences (or have set it via `identify` / `changeTenant`), make sure that tenant is included in the `scope.tenant_id` key while creating [userToken](/docs/client-authentication) passed during identifying user, else an error will be thrown due to scope mismatch.
</Warning>
