...
Before you send to Microsoft Teams, create a Teams app and save its **App ID**, **password**, **App Type**, and **Tenant ID** on the Microsoft Teams vendor form in SuprSend.

For bot delivery you also wrap the bot in a Teams app package that customers install. That's what makes the message show *your* name and icon, and what lets you DM people. In the [customer's Teams app](/docs/microsoft-teams-customer-bot) use case they create their own copy.
...
Your app package is ready. To test it, upload it in your own tenant from **Teams admin center → Teams apps → Manage apps → Upload new app**. How customers install it is covered on [Send using your Teams app](/docs/microsoft-teams-customer-workspaces).
...
Go to **Vendors → Microsoft Teams** ([open vendor settings](https://app.suprsend.com/en/staging/vendors/ms_teams/ms_teams-ms_teams?tenant_id=default)). Add the **Application (client) ID** and the **client secret** (Password field).
...
After you save, SuprSend tries to log in to Microsoft with these credentials and shows the result in the **Access Token** field on the same form.

If the field stays **empty**, the login failed. The usual cause is a wrong **Tenant ID** — it must be the directory the bot was created in, not a customer's.

If the field is **filled**, the login worked. Sends can still fail if **App Type** doesn't match the Azure Bot, so send one test message from your use-case page before going further. A `401 Authorization has been denied for this request` on that test means App Type is wrong.
...
If an enterprise customer brings their own bot, those credentials go on a [tenant-level vendor](/docs/tenant-vendor) instead. See [Send via your customer's Teams app](/docs/microsoft-teams-customer-bot).
