---
title: "Postgres"
description: "Connect your Postgres database to SuprSend to auto sync users, build recurring cohort lists, and trigger broadcast notifications from your warehouse data."
---
...
<Frame>
  <img src="/images/docs/5c68e43-image.png" />
</Frame>
...
| Password      | Database password created for suprsend sync. We recommend using a user with the minimal privileges here. This user only requires read permissions with access limited to the tables you want to sync from. **Note:** Passwords are treated as strings. If your password contains special characters, ensure they are properly escaped. |
...
If your Postgres database requires IP whitelisting, you need to whitelist the following SuprSend IP addresses:

- `3.6.60.56`
- `3.111.232.56`

Add these IP addresses to your database's firewall or security group settings to allow connections from SuprSend.

<Note>
  **IP Address Updates**: These are public-facing IP addresses used by SuprSend to connect to customer databases. If you encounter connection issues or need to verify the current IP addresses, please contact [support@suprsend.com](mailto:support@suprsend.com).
</Note>

<Warning>
  **Connection String and Password Handling:**

  - **Special Characters in Passwords:** If your database password contains special characters (such as `@`, `#`, `$`, `%`, `&`, `*`, etc.), ensure they are properly escaped in the connection string. Character escaping issues in the connection string can cause connection failures or query errors even after a successful initial connection.

  - **Password Type:** Database passwords are always treated as strings, not integers. If you encounter errors related to password type checking, ensure your password is provided as a string value.
</Warning>
