...
<Tip>
  **AI prompt — generate email HTML:** *"Generate a responsive HTML email template for [describe use case, e.g. order shipped notification]. Use these variables: [list variables with sample values]. Use table-based layout, inline CSS, and web-safe fonts. Make it work across Gmail, Outlook, and Apple Mail."*
</Tip>
...
Email Markup adds rich interactive features in Gmail — order tracking cards, one-click RSVP, flight boarding passes — without opening the email.

Add JSON-LD to the **Email Markup JSON** field in Email Settings. Supports Handlebars variables.
