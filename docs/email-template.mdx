...
To adjust the width of your email template for desktop viewing, go to **Body** → **Content Width** option and adjust the width value as needed.
...
<Steps>
  ...
  <Step title="Using variables in the templates:" stepNumber="2">
    Once the variables are declared, you can use them while designing template for any channel. We support `handlebarsjs` to add variables in the template. As a general rule, all the variables have to be entered within double curly brackets: `{{variable_name}}`. For URLs and values which can have special characters, you can use triple curly braces `{{{url}}}` to avoid HTML-escaping.
...
<Accordion title="How to prevent HTML injection when providing variables to my emails?" defaultOpen={false}>
      Double curly brackets `{{ var }}` HTML-escape values by default, which helps prevent injection. Use triple curly brackets `{{{ var }}}` only when you need raw HTML and have special charcaters in the value (like &, ?, =, etc.) and can covert into random strings (like &amp;, ??, =??, etc.) after HTML-escaping.
</Accordion>
***
