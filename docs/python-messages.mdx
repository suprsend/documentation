...
The Messages API lets you fetch a paginated list of messages in your workspace and bulk update message statuses (e.g. mark as seen, read, archived). These Python helpers are wrappers over the underlying [List Messages](/reference/list-messages) and [Bulk Update Message Status](/reference/bulk-update-messages) endpoints.
