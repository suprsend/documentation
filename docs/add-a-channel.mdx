  ...
  <Tab title="Microsoft Teams">
    ...
    <Steps>
      <Step title="Open the channel's Workflows menu">
        In Teams, go to the channel you want to post into, select **More options (…)** next to it, and
        choose **Workflows**.
      </Step>

      <Step title="Pick the webhook template">
        Choose **Send webhook alerts to a channel**, fill in the parameters, and **Save**.
      </Step>

      <Step title="Copy the URL">
        The workflow generates a webhook URL once saved. That channel is the only one it can post to.
      </Step>
    </Steps>
    ...
    <Accordion title="Or install a Teams app — any channel, and DMs">
      Install your Teams app to your tenant, then store `tenant_id`, `service_url` and
      `conversation_id` for a channel, or `user_id` in place of `conversation_id` for a DM.

      `service_url` is regional — `https://smba.trafficmanager.net/amer` is one region, not a universal
      endpoint. All three values have to come from the same tenant.
    </Accordion>
  </Tab>
</Tabs>
  ...
  <Tab title="Dashboard">
    <Steps>
      ...
      <Step title="Pick an editor">
        Both channels offer two, and you can switch at any time — each keeps its own content. **Text**
        for Slack and **Markdown** for Teams handle a plain line like this one. **JSONNET** outputs
        Slack [Block Kit](/docs/slack-template) or Teams [Adaptive Cards](/docs/ms-teams-template),
        which is what you want as soon as the message needs buttons, sections or images.
      </Step>
