...
<CodeGroup>
  ...
  <img src="/images/add-channel-apns-vendor.png" alt="SuprSend iOS APNs vendor form with fields for nickname, mode or environment, p8 file upload, auth key ID, team ID and bundle ID" />
</CodeGroup>
...
| **Bot token** | One token for your workspace | Any channel or person in that workspace | Yes |
| **OAuth install** | A token per customer | Any channel or person in each customer's workspace | Yes |
...
<Tabs>
  <Tab title="Incoming webhook">
    **Slack**
    ...
    3. Click **Add New Webhook to Workspace**, pick the channel to post into, and **Allow**.

       <Frame>
         <img src="/images/add-channel-slack-authorize.png" alt="Slack authorization screen asking which workspace and channel the webhook should post to" />
       </Frame>

    4. Copy the URL. It looks like `https://hooks.slack.com/services/TXXXX/BXXXX/XXXXXXX`.

       The channel you pick here is the only one this URL can ever post to. To reach a second channel,
  ...
  <Tab title="Bot token">
    Same Slack app as the webhook tab — you're just taking a different credential out of it. Add the
    OAuth scopes you need, install the app to your workspace, and copy the bot token. It starts with
    `xoxb-`.
