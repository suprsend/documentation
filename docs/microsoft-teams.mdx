...
## Create Teams app
    ...
    <Steps>
      ...
      <Step title="Add a name and other details">
        ...
        <Frame>
          <img src="/images/docs/fe39ac8-image.png" />
        </Frame>
      </Step>
      ...
      <Step title="Get started with the Azure Portal">
        ...
        <Frame>
          <img src="/images/docs/1aa2a33-image.png" />
        </Frame>
      </Step>
      ...
      <Step title="Navigate to Authentication from the left panel">
        ...
        <Frame>
          <img src="/images/docs/775bfa0-Screenshot_2024-07-04_at_7.36.47_PM.png" />
        </Frame>
      </Step>
      ...
      <Step title="Add your Recently Created Bot">
        ...
        <Frame>
          <img src="/images/docs/07393df-image.png" />
        </Frame>
      </Step>
      ...
      <Step title="Add specific info about your bot, and add some permissions">
        ...
        <Frame>
          <img src="/images/docs/cce58ed-image.png" />
        </Frame>
      ...
      <Step title="Add the Client ID">
        ...
        <Frame>
          <img src="/images/docs/6e1bcf0-Screenshot_2024-04-04_at_3.24.04_AM.png" />
        </Frame>
  ...
  ## Add Teams integration on SuprSend
    ...
    <Frame>
      <img src="/images/docs/0f6b463-image.png" />
    </Frame>
  ...
  ### App Type and Tenant ID
    ...
    **App Type** must match how your bot is registered in Azure:
    ...
    Find it in the Azure portal under **Microsoft Entra ID → Overview → Tenant ID**, or on your app
    ...
    <AccordionGroup>
      <Accordion title="Sending a direct message" defaultOpen={false}>
        ...
        **Getting tenant ID, user ID, conversation ID:**
        ...
        <Tip>
          The roster returns two identifiers per member, and they are not equivalent:

          - **The Entra object ID** — returned as `aadObjectId` by `/v3/conversations/{id}/members`
            and as `objectId` by `/v3/conversations/{id}/pagedmembers`. Stable for the life of the
            user.
          - **`id`** (`29:…`) — an encrypted identifier scoped to *one bot app registration*. It
            works as `user_id`, but if you ever recreate or replace your bot, every stored `29:`
            value stops resolving.

          Prefer the object ID. Note that it comes back as `null` unless your Teams app manifest
          declares a resource-specific consent permission such as `TeamMember.Read.Group`. Without
          that, `pagedmembers` still returns `userPrincipalName`, which you can resolve to an object
          ID via Microsoft Graph or look up in the Azure portal under **Microsoft Entra ID → Users**.
        </Tip>
      ...
      <Accordion title="Sending to a channel" defaultOpen={false}>
        ...
        <Frame>
          <img src="/images/docs/12077b8-image.png" />
        </Frame>
      ...
      <Accordion title="Sending message using incoming webhook" defaultOpen={false}>
        ...
        **Get Incoming webhook for a teams channel:**
        ...
        <Warning>
          Office 365 connectors, the older route shown below, are retired — Microsoft began the
          rollout on 18 May 2026, and connector URLs stop delivering once it reaches a tenant. The
          `incoming_webhook` profile accepts either URL format, so migrating a subscriber means
          replacing the URL and nothing else. Workflows supports MessageCard as well as Adaptive
          Card, so your existing card templates carry over. The steps below are kept only for
          reference while you migrate.
        </Warning>

        To get an incoming webhook the older way, select **connectors** from channel -> more options.

        <Frame>
          <img src="/images/docs/f99ea8e-image.png" />
        </Frame>

        Configure **Incoming webhook** in connectors. Click on create and copy the webhook URL

        <Frame>
          <img src="/images/docs/a6006b1-image.png" />
        </Frame>
      </Accordion>
    </AccordionGroup>
