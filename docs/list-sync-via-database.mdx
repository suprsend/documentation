...
description: "Automate user list sync in SuprSend by connecting your data warehouse and writing SQL queries to refresh subscriber lists for campaigns and broadcasts."
---
...
<Steps>
  ...
  <Step title="Write SQL query to extract and load data">
    ...
    <Frame>
      <img src="https://files.readme.io/0bc69e9-query_editor.gif" />
    </Frame>
    ...
    **Query Configuration Tips:**

    - **Column Naming:** Use consistent column names in your query. Column names are case-sensitive when mapping to user properties or channels. For example, if your query returns `Email`, `email`, and `EMAIL` as different columns, they will be treated as separate columns.

    - **Phone Number Formatting:** If you're mapping phone numbers to SMS or WhatsApp channels, ensure your query returns phone numbers in E.164 format (`+<country-code><number>`). You can format phone numbers directly in your SQL query:
      - **PostgreSQL:** `SELECT '+91' || phone_number AS phone FROM users`
      - **MySQL:** `SELECT CONCAT('+91', phone_number) AS phone FROM users`
      - If all your users are from the same country, you can prepend the country code in the query. For example, `+91` for India or `+1` for US.

    - **Channel Mapping:** When mapping columns to communication channels (email, SMS, WhatsApp), ensure the column values are in the correct format:
      - **Email:** Standard email format (e.g., `user@example.com`)
      - **SMS/WhatsApp:** E.164 format with country code (e.g., `+919999999999`)

    - **Multiple Channels:** To map multiple channels for the same user, include separate columns for each channel in your query (e.g., `email`, `phone_sms`, `phone_whatsapp`).
  ...
  <Step title="Save query and commit to setup sync frequency">
    Once you are satisfied with your query results, `Save query` and `Commit changes` it. You'll see sync settings on commit.

    <Frame>
      <img src="/images/docs/4173882-image.png" />
    </Frame>
    ...
    | Column header                | This is the column name from your query output used to map user property. **Important:** Column names are case-sensitive. The column name you enter here must exactly match (including case) the column name returned by your SQL query. A case mismatch will prevent the column from being mapped correctly, causing email, SMS, or WhatsApp channels to fail syncing.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
    | is channel?                  | enable it if the column value is used to update communication channel such as `email`, `sms` or `whatsapp` in user profile. **Note:** For SMS and WhatsApp channels, phone numbers must be in [E.164 format](https://www.twilio.com/docs/glossary/what-e164) (`+<country-code><number>`). For example, `+919999999999` for India or `+15555555555` for US. If your database stores phone numbers without country codes, you'll need to prepend the country code in your SQL query using string concatenation (e.g., `CONCAT('+91', phone_number)` in MySQL or `'+91' || phone_number` in PostgreSQL).                                                                                                                                                                                                                                                                                                                                                                                  |
    ...
    <Frame>
      <img src="/images/docs/4d52dc3-image.png" />
    </Frame>
...
When you enable a sync, we’ll resume the sync. The next sync interval will send all data that has changed since the last sync. `{{last_sync_time}}` variable will also store the timestamp of your last successful sync. So, when you enable it, it will sync all the data that has changed since the last sync.

<Frame>
  <img src="/images/docs/a27a8d1-image.png" />
</Frame>
...
<Frame>
  <img src="/images/docs/28f4dbf-image.png" />
</Frame>
...
<Frame>
  <img src="/images/docs/59235f7-image.png" />
</Frame>
...
<Frame>
  <img src="/images/docs/7c9845b-image.png" />
</Frame>
