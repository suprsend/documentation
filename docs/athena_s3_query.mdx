...
<Warning>
**Connectors created before 21 May 2026** run on `lz4` + `shared` by default, and neither setting is compatible with this guide. Open the connector, switch the codec to anything other than `lz4`, and set the path layout to `per_type`.
