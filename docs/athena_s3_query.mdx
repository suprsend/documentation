...
<Warning>
**Connectors created before 21 May 2026** run on `lz4` + `shared` by default, and neither setting is compatible with this guide. To switch, write to [support@suprsend.com](mailto:support@suprsend.com)—we'll help you change the codec to a non-`lz4` option and move the path layout to `per_type`.

The new settings apply only to files written **after** the switch—everything already in your bucket keeps its original codec and folder. To work with the historical data, either scope queries to the post-switch date range using the `year`/`month`/`day`/`hour` filters, or ask support to rewrite the existing files.
</Warning>
...
- Timestamp-as-string columns (`success_achieved_at`): parse with `from_iso8601_timestamp(column)` or `date_parse(column, '<format>')` depending on the format your data is written in.
