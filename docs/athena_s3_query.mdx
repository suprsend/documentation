...
- **Compression: any codec except `lz4`.** Athena can't reliably read `lz4`-encoded Parquet—this is a known Athena limitation, not something specific to SuprSend. `snappy` (the default), `gzip`, `zstd`, `brotli`, and `none` all work.

You can review or change either setting in [Compression and path layout](/docs/amazon_s3_v2#compression-and-path-layout).

<Warning>
**Connectors created before 27 May 2026** run on `lz4` + `shared` by default, and neither setting is compatible with this guide. Open the connector, switch the codec to anything other than `lz4`, and set the path layout to `per_type`.

The new settings apply only to files written **after** the switch—everything already in your bucket keeps its original codec and folder. To work with the historical data, either scope queries to the post-switch date range using the `year`/`month`/`day`/`hour` filters, or contact support to rewrite the existing files.
</Warning>
