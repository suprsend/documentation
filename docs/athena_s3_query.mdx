---
title: "Query S3 data with Amazon Athena"
...
- **Cost** — Athena charges per terabyte scanned. Always include partition filters and `SELECT` only the columns you actually need.
- **Freshness** — The connector rewrites hourly Parquet files when data updates. The next Athena query sees the new state, no refresh step needed.
- **Cast where you need typed values** — Several columns are stored as `string` even though they represent JSON, booleans, integers, or timestamps. Use `CAST(... AS BIGINT)`, `from_iso8601_timestamp(...)`, or `json_extract(...)` in your queries (see the [table schemas](/docs/amazon_s3_v2#table-schema) for which columns this applies to).
- **Changed the S3 path?** If you reconfigure the connector to write to a different bucket or prefix, drop and recreate each table with the new `LOCATION` and `storage.location.template`.
