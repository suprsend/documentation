...
| Recipient      | Recipient properties. It is the `distinct_id` in your event trigger or the key value defined in [override recipient](/docs/override-recipient-list) field. For inline workflow trigger, it is the `distinct_id` in recipient object. **Note:** User preferences are not available in workflow payload. If you need to use preference-based logic, store preferences as custom properties in the user profile and reference them here. | Directly specify key if you are adding this in key or add as `$recipient.<property_key>` if you add it in value field. |
