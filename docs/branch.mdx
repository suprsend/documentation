---
title: "Branch"
description: "Use the branch node in SuprSend workflows to route notifications through different paths with if, else if, and else conditions evaluated on input data."
---
...
| **Tenant** | Properties of the tenant/brand | Add as `<property_key>` for reserved properties,  `properties.<property_key>` for custom properties | Add as `$tenant.<property_key>` or `$tenant.properties.<property_key>` |
...
| Tenant | Add as `$tenant.<property_key>` for reserved properties, add as `properties.<property_key>` for custom properties (for example, `$tenant.timezone`, `properties.timezone`) |
...
<br/>
**Condition Syntax:**<br/>
Condition on message status looks like this<br/> `<node slug> IS/ IS_NOT <message status>`. Eg. `message_status from node "welcome-email" is "seen"`.
...
<Frame caption="Message Status condition on UI">
  <img src="/images/branch-message-status.png" />
</Frame>
<br/>
**Message Status and their meaning:**<br/>
...
<AccordionGroup>
  ...
  <Accordion title="What's the difference between 'delivery_failed' and 'is_not delivered'?" defaultOpen={false}>
  ...
  <Accordion title="Where do I find the node reference for Message Status?" defaultOpen={false}>
    ...
    <Frame>
      <img src="/images/node-slug.png" />
    </Frame>
    <Frame>
      <img src="/images/node-slug-edit.png" />
    </Frame>
  </Accordion>
</AccordionGroup>
