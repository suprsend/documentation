...
- **Every 5 minutes**, SuprSend syncs your notification data to S3 in Parquet format. Data lands in hourly partitions across three data points, depending on your connector settings. With the default path layout (`per_type`), files are organized like this:
  ...
  You can switch to a `shared` layout where all data points share the same date-partitioned path—see [Compression and path layout](#compression-and-path-layout).
- At every sync, we **add or replace the existing hourly parquet files** where the data has been changed.
  ...
  - For data warehouses that don’t support automatic row overwrites (for example, BigQuery), use the updated_at column to select the latest state of the data.
- **Data is encrypted with TLS 1.2+** in transit and **SSE-S3/SSE-KMS** at rest. 
- The Parquet format and partition structure work natively with query engines like Athena, Spark, and Presto. See [Query S3 export with Amazon Athena](/docs/athena_s3_query) for the table DDL and a sample query.
...
| channel_info | Channel-specific value (for example, email address) | string |
...
When you add or edit the connector, you can configure how Parquet files are written to your bucket.

<Note>
**Already using the connector?** Connectors configured before these options were introduced default to **`lz4` compression** and the **`shared` path layout**. New connectors default to `snappy` + `per_type`. If you plan to query your data with Amazon Athena, edit your connector and switch compression to `snappy` (or `gzip`/`zstd`/`brotli`/`none`) and path layout to `per_type`—see the [Athena setup guide](/docs/athena_s3_query) for why.

Changing these settings only affects **files written after the change**. Files already in your bucket keep their original codec and folder structure, so you'll either need to scope queries to the post-switch date range or rewrite/backfill the historical files into the new layout.
</Note>

### Compression

Parquet files are compressed using the codec you select. **`snappy` is the recommended default**—it's fast and broadly supported by query engines and warehouses. Other codecs are available if you have a specific need.
