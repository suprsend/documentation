...
SuprSend syncs your notification data to S3 every **5 minutes** as Parquet files, partitioned by hour. With the default `per_type` path layout, your bucket ends up structured like this:
...
- **Parquet is read natively** by every common query engine and warehouse. For Athena specifically, see [Query S3 data with Amazon Athena](/docs/athena_s3_query).
...
- The path layout and compression codec are configurable—see [Compression and path layout](#compression-and-path-layout).
...
<Note>
**On a connector created before these options existed?** It still uses the previous defaults: **`lz4` compression** and the **`shared` path layout**. New connectors default to `snappy` + `per_type`. Changing either setting on an existing connector only affects files written **after** the change; existing files keep their original codec and folder.
</Note>

### Compression

Pick the codec the Parquet files are encoded with. **`snappy` is the default** and works well for most query engines and warehouses.

| Option | Default | Notes |
|--------|:-------:|-------|
| `snappy` | ✅ | Fast compression and decompression; balanced choice for analytics workloads. |
| `lz4` |  | Faster than Snappy, slightly larger files. **Not compatible with Amazon Athena**—Athena can't read `lz4`-compressed Parquet reliably (a generic Athena limitation). |
| `gzip` |  | Smaller files, slower to read and write. |
| `zstd` |  | Strong compression ratio with read/write speeds comparable to gzip. |
| `brotli` |  | Highest compression ratio in this list; slower to read. |
| `none` |  | No compression. Use only if your downstream system requires uncompressed Parquet. |
...
**Recommended for every query engine and warehouse.** Each data point lives in its own prefix, so tables, schemas, and IAM scopes stay cleanly separated—a one-to-one mapping between table and folder. **Required if you're using Amazon Athena**—see [Query S3 export with Amazon Athena](/docs/athena_s3_query).
...
For the full DDL, partition-projection settings, and example queries (including a join that traces a single request across all three tables), follow [Query S3 data with Amazon Athena](/docs/athena_s3_query).

<Note>
Athena only works with `per_type` path layout and a non-`lz4` compression codec. Both are the defaults for new connectors; existing connectors created before these options were introduced need to be reconfigured first—details in the [Athena guide](/docs/athena_s3_query#prerequisites).
</Note>
...
In **Settings** → **Connectors** → **Amazon S3 v2.0**, you should see:
