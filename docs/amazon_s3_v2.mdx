...
Parquet files are compressed using the codec you select. The codec only affects how the file is encoded on disk; pick one your downstream query engine or warehouse supports.

| Option | Default | Notes |
|--------|:-------:|-------|
| `snappy` | ✅ | Fast compression and decompression; balanced choice for analytics workloads. |
| `lz4` |  | Faster than Snappy, slightly larger files. **Not recommended if you plan to query with Amazon Athena**—Athena doesn't read lz4-compressed Parquet reliably. |
...
Use this when you want each data point as a separate table in Athena, Glue, Spark, or BigQuery—each table points at its own prefix.
...
Use this if you already have a pipeline that consumes a single date-partitioned prefix.
