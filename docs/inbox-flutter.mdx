...
description: "Integrate the SuprSend Inbox in your Flutter app using the headless SDK and hooks to fetch notifications, mark as read, and build your own custom inbox UI."
---

<Warning>
  **This page covers Flutter only.** For **React / Web** applications, use [`@suprsend/react`](/docs/react-inbox-integration) (drop-in components) or [`@suprsend/react-core`](/docs/react-sdk-headless-feed) (headless) — both use JWT authentication and include interaction-observer based seen tracking. Do **not** install the legacy `@suprsend/react-inbox` package for web React apps.

  The Flutter Inbox SDK currently uses HMAC `subscriber_id` authentication as documented below.
</Warning>
