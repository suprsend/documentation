...
tag: "DEPRECATED"
description: "Generate HMAC signatures to securely authenticate users and subscriber IDs in headless Inbox implementations, covering hash keys, payloads, and code samples."
---

<Warning>
  **For React/Web apps, use JWT authentication instead.** The `@suprsend/react` (drop-in) and `@suprsend/react-core` / `@suprsend/web-sdk` (headless) SDKs authenticate users with signed JWTs — see the [client authentication guide](/docs/client-authentication) and the [React migration guide](/docs/react-migration-guide).

  This page documents the legacy HMAC `subscriber_id` flow that is still used by the React Native (`@suprsend/react-headless`) and Flutter Inbox SDKs. Do **not** use this for new React/Web integrations.
</Warning>
...
<Frame>
  <img src="/images/subscriberslistingpage.png" />
</Frame>
