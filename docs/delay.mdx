...
There are 3 types of delay types available: **Fixed** (fixed for all users), **Dynamic** (Passed in trigger payload), Relative (relative to a future timestamp, for example: 10 mins before task due time).
...
<AccordionGroup>
  ...
  <Accordion title="Relative">
    ...
    - **Timestamp**: An ISO-8601 format date-time (for example:, 2024-03-02T20:34:07Z), which must be in the future.
