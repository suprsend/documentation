...
<Info>📘 Available in SDK version >= 1.10.0</Info>
...
```javascript Request
const workflow = new WorkflowTriggerRequest(workflow_body, {
  tenant_id: "tenant_id1",
});
```
...
// workflow1 instance
const wf1 = new WorkflowTriggerRequest(wf_body1, {
  tenant_id: "tenant_id1",
  idempotency_key: "_unique_identifier_of_the_request_",
});
// workflow2 instance
...
<Warning>
  A single workflow instance size (including attachment) must not exceed 800KB
  (100 x 1024 bytes).
</Warning>
