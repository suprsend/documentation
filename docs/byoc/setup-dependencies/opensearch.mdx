...
description: "Set up OpenSearch in Kubernetes for SuprSend self-hosted deployments — operator, cluster, dashboards, and Helm configuration."
---
...
Create a cluster definition file `suprsend-opensearch.yaml`:
...
You can also try making a curl request from within the cluster:

```bash
# Replace the password with the one you configured
...
<Note>
This section shows only the **OpenSearch-specific** configuration. You must also configure other required secrets and values for SuprSend to work properly. See the complete configuration guide: [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide)
</Note>

### Kubernetes Secret Configuration

First, add the OpenSearch-specific secret to your SuprSend secrets file (see [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide) for the full secrets template):
...
```yaml
# connection url for opensearch (key name in secret)
...
<AccordionGroup>
  <Accordion title="Storage and scaling">
    Use **dedicated storage classes** (e.g., SSD-backed volumes). Enable **auto-scaling** and **snapshot policies**.
  </Accordion>
  <Accordion title="High availability">
    Run OpenSearch in **HA mode** with 3+ nodes for redundancy.
  </Accordion>
  <Accordion title="Access and TLS">
    Use **Ingress or LoadBalancer** for external access with TLS termination. Configure proper **TLS certificates** for production deployments.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols="2">
  <Card title="Temporal Setup Guide" icon="clock" iconType="solid" href="/docs/byoc/setup-dependencies/temporal">
...
- [SuprSend Docs](https://docs.suprsend.com)
- [OpenSearch Configuration Reference](https://opensearch.org/docs/latest/)
