...
description: "Set up a production-ready ClickHouse cluster on Kubernetes with the Altinity Operator for SuprSend self-hosted deployments."
---

This guide walks you through setting up a production-ready ClickHouse cluster on Kubernetes using the **Altinity ClickHouse Operator**. It also includes steps to test **data persistence** and **connectivity** to ensure your deployment is reliable.
...
Generate a password hash for your default user:
...
<Note>
The `chi` prefix stands for **ClickHouseInstallation** (CRD name defined by the Altinity Operator).
</Note>
...
### Option 2: Using clickhouse-client (recommended for testing)
...
4. Reconnect and check the data:
   ```bash
...
<Warning>
Use caution: deleting PVCs will erase all stored data if reclaim policy is not set to `Retain`.
</Warning>
...
| Purpose | Command |
|---------|---------|
| List databases | `kubectl exec -it chi-suprsend-ch-suprsend-ch-0-0-0 -n clickhouse -- clickhouse-client -q "SHOW DATABASES"` |
| Check system tables | `kubectl exec -it chi-suprsend-ch-suprsend-ch-0-0-0 -n clickhouse -- clickhouse-client -q "SELECT name, engine FROM system.tables WHERE database='test'"` |
| View pod logs | `kubectl logs chi-suprsend-ch-suprsend-ch-0-0-0 -n clickhouse` |
...
<AccordionGroup>
  <Accordion title="Pods stuck in Pending">
    **Possible cause**: StorageClass not found. **Fix**: Check `storageClassName` matches your cluster.
  </Accordion>
  <Accordion title="Pod restarting">
    **Possible cause**: Wrong image or insufficient resources. **Fix**: Check logs and resource limits.
  </Accordion>
  <Accordion title="Can't connect via port-forward">
    **Possible cause**: ClickHouse not yet ready. **Fix**: Wait until log shows "Ready for connections".
  </Accordion>
  <Accordion title="Data lost after restart">
    **Possible cause**: PVC deleted or `Retain` not set. **Fix**: Ensure `reclaimPolicy: Retain`.
  </Accordion>
</AccordionGroup>
...
<Note>
This section shows only the **ClickHouse-specific** configuration. You must also configure other required secrets and values for SuprSend to work properly. See the complete configuration guide: [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide)
</Note>

### Kubernetes Secret Configuration

First, add the ClickHouse-specific secrets to your SuprSend secrets file (see [SuprSend Installation Guide](/docs/byoc/suprsend-installation-guide) for the full secrets template):
...
<CardGroup cols="2">
  ...
  <Card title="List of Dependencies" icon="server" iconType="solid" href="/docs/byoc/list-of-dependencies">
    Review other infrastructure components.
  </Card>
</CardGroup>
