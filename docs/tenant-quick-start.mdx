  ...
  <Tab title="SuprSend UI">
    <Steps>
      <Step title="Select Tenants tab and create New Tenant">
        Select **Tenants** tab from the side navigation and click on **New Tenant**. Enter a **Tenant ID**, generally the customer ID from your system and a **Tenant Name**, generally the company name, then confirm.
...
Use custom properties to conditionally show content for specific tenants. Set a property (for example, `pricing_type: free`) on the tenant, then basis that, show upgrade banner in the template. In email template, you can use [display condition](/docs/email-template#display-email-blocks-based-on-condition) to show/hide the content based on the property.
