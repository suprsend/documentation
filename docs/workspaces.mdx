...
<AccordionGroup>
  <Accordion title="Sandbox" defaultOpen={true}>
    ...
    - To prevent accidental spam, **Email, SMS, and WhatsApp** notifications can only be delivered to [verified channels](https://app.suprsend.com/en/staging/developers/verified-channels) (up to 5 per channel type). Slack, Teams, Inbox, and Push are unrestricted. Reach out to us if you need to raise the limit.
...
<AccordionGroup>
  ...
  <Accordion title="How should I model multi-region workspaces?">
    You can create custom staging and production workspace for each of the data region. So, you can have US-Staging and US-Production for US data region and EU-Staging and EU-Production for EU data region.
  </Accordion>
