...
**Note:** Since React SDK is built on the web SDK base, you can pass the `locale` parameter through the `getPreferences` method to get translated category names and descriptions. See the [web SDK documentation](/docs/js-preferences#get-preferences-data) for details.
