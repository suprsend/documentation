...
<Steps>
  ...
  <Step title="Variant selection">
    If [variants](/docs/template-variants) are configured, SuprSend evaluates variant conditions against the trigger payload and recipient profile. The first matching variant is used; if nothing matches (or no variants are set up), the `default` variant is sent.
  </Step>
  <Step title="Render">
