...
<Steps>
  ...
  <Step title="Optional: Better attachment MIME type detection">
    When adding file attachments, the SDK needs to detect the file's MIME type (e.g. `application/pdf`, `image/png`). By default, it uses Python's built-in `mimetypes` module, which guesses based on the file extension.
