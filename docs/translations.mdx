---
title: "Translations"
description: "Use translations in SuprSend to localize notifications per user locale, keeping one template across languages instead of duplicating templates per language."
---
...
You can use translations in your templates using the `t` tag. Anything inside the `t` tag will be replaced with the translation for the key. Handlebars syntax, lookup order, and error cases are also listed on the [`t` helper](/docs/handlebars-helpers#t).
