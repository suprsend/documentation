...
<Update label="20 Feb 2025">
  ...
  We’ve recently been hearing multi-tenant use-cases where a user belong to multiple tenants and would want to see Inbox feed for all tenants in a single product. For example, an account manager is handling multiple client accounts and need to see updates or daily reports linked to all their accounts in a single feed.
...
<Update label="31 Jan 2025">
  ...
  🔹 Depth 0 → Notify only the object’s channels (for example, Slack team, shared inbox).
...
<Update label="27 Nov 2024">
  ...
  **A common use case is escalation workflows**—for example, if a team member doesn't take action within a set time frame, the workflow escalates the issue and notifies their manager.
...
<Update label="17 Nov 2024">
  ...
  2. **Workflow Step-based segmentation** - Another use case is dynamically adding or removing a user from the list when they complete a workflow step. For example, in a knowledge series designed to onboard new users, remove a user from the POC list once they complete onboarding steps.
</Update>
...
<Update label="14 Nov 2024">
  ...
  - **Cross-Product Identity Consolidation**: When users interact across multiple products (for example, different apps or services within your platform), they may have different identifiers for each product which needs to be merged later.
...
<Update label="7 Nov 2024">
  ...
  - **Send notifications to non-user entities like group emails, Slack channels, or shared inboxes** (for example, a Notion feed). Ideal for SaaS applications sending account-level alerts (for example, anomaly notifications) to shared channels. Objects can have it's own channels and preferences to handle this use-case.
...
<Update label="3 Nov 2024">
  ## Date-Time comparators in workflow conditions

  You can now compare date-time fields in [workflow conditions](https://docs.suprsend.com/docs/branch#key-value-pair). This lets you compare two timestamps where values can be:

  - **Variable**–computed from workflow input data
  - **Static**–a fixed timestamp (for example `2024-01-01T00:00:00Z`)
  - **Relative to current timestamp**–for example: "`now`" or "`now+30d`" (current timestamp \+/- interval). Current timestamp is calculated at node runtime and is timezone aware.
...
<Update label="27 Aug 2024">
  ...
  - Add conditions on event properties using a simple key-operator-value expression (for example: `booking_id = 123`). Add condition on multiple event properties using `AND`,`OR`.
  - Apply conditions across multiple events (for example, avoid sending a notification if a user completes an action or achieves a specific milestone).
